...
public class QDDataManager {
    ...
    private List<Class<? extends BaseFragment>> mStatsNames;
    ...
    public List<QDItemDescription> getStatsDescriptions() {
        List<QDItemDescription> list = new ArrayList<>();
        for (int i = 0; i < mStatsNames.size(); i++) {
            list.add(mWidgetContainer.get(mStatsNames.get(i)));
        }
        return list;
    }
