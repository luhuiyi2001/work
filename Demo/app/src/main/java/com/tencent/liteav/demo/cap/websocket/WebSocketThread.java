...
public class WebSocketThread extends Thread {
     ...
     * 2-已连接
     */
    private int connectStatus = ConnectStatus.DISCONNECT;
...
//        mHandler.sendEmptyMessage(MessageType.CONNECT);
    ...
    private class WebSocketHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            CLog.d(TAG, "handleMessage : " + msg.what);
            switch (msg.what) {
                ...
                case MessageType.SEND_MESSAGE:
                    if (msg.obj instanceof String) {
                        ...
                        if (mWebSocket != null && connectStatus == ConnectStatus.CONNECTED) {
                            send(message);
                    ...
                    if (mWebSocket == null) {
                        ...
                        mWebSocket = new WebSocketClient(new URI(connectUrl), new Draft_6455()) {
                            ...
                            public void onMessage(String message) {
                                connectStatus = ConnectStatus.CONNECTED;
                                CLog.d(TAG, "WebSocket 接收到消息：" + message);
                            ...
                            public void onError(Exception ex) {
                                CLog.e(TAG, "WebSocketClient#onError(Exception)", ex);
                            }
                        };
                        CLog.d(TAG, "WebSocket 开始连接...");
                        mWebSocket.connect();
                    } else {
                        CLog.d(TAG, "WebSocket 开始重新连接...");
                        mWebSocket.reconnect();
                    }
                } catch (URISyntaxException e) {
                    connectStatus = ConnectStatus.DISCONNECT;
                    CLog.d(TAG, "WebSocket 连接失败");
                    if (mSocketListener != null) {
        ...
        private void disconnect() {
            ...
            if (connectStatus == ConnectStatus.CONNECTED) {
                CLog.d(TAG, "正在关闭WebSocket连接");
                if (mWebSocket != null) {
                    mWebSocket.close();
                }
                connectStatus = ConnectStatus.DISCONNECT;
                CLog.d(TAG, "WebSocket连接已关闭");
            }
        }

        private void send(String text) {
            CLog.d(TAG, "send = " + text);
            if (mWebSocket != null && connectStatus == ConnectStatus.CONNECTED) {
                try {
                    mWebSocket.send(text);
                    CLog.d(TAG, "数据发送成功：" + text);
                } catch (WebsocketNotConnectedException e) {
                    connectStatus = ConnectStatus.DISCONNECT;
                    CLog.e(TAG, "send()" + text, e);
                    CLog.e(TAG, "连接已断开，数据发送失败：" + text, e);
                    if (mSocketListener != null) {
                        mSocketListener.onDisconnected();
